...
// guarantees a normal String completion, and the two refusals that
// stand ahead of both (a regex `replaceAll` without `g`, whose clause
// throws, and a function replacement, whose claim has no wire form).
...
func TestEffectReplace_AFunctionReplacementStaysRefused(t *testing.T) {
	kernel := kernelDelegationLoadKernel(t)
	// the replacer is caller code: it can throw and it can write, and
	// the kernel's sequence route has no spelling for "a word, or a
	// thrown exit" — the refusal answers top, which admits both
	replaceRefused(t, kernel, `out = s.replace("a", (m) => m);`)
}
