 ...
 * schema (refinementsets/json_case_grammar.go): the CLASSIFICATION
 * pins (which arm a case kind reaches) live in refinementsets' own
 * json_case_grammar_test.go, kernel-free — these are the MEMBERSHIP
 * pins, against a real loaded kernel's Member ask (x ∈ A), the same
 * idiom this file's own jsonGrammarAdmits helper already applies to
 * the number-only shape.
 */
